...
class SensorScaler
{
	public:
		...
		byte SensorNumber;				// Which sensor will control
		byte TargetPulseNumber;			// Which pulse will be controlled
		unsigned long ZeroPulseDelay	// Which pulse delay corresponds to sensor value =0; Default value =-1 - do not change delay
		unsigned long ZeroPulseTime		// Which pulse duration corresponds to sensor value =0; Default value =-1 - do not change duration
		unsigned long ZeroPulsePause	// Which delay between pulses corresponds to sensor value=0; Default value =-1 - do not change delay
	...
	Impulse(byte _SensorNumber, byte _TargetPulseNumber	, unsigned long _ZeroPulseDelay,unsigned long  _ZeroPulseTime, unsigned long  _ZeroPulsePause, double _StartScaling, double _TimeScaling, double _PauseScaling)
	{
		Scaling=true;
	...
	ScaleImpulse(byte Sensor, Impulse Target)
	{
		if (Scaling)
			...
			{
				if (StartScaling!=0)
					if (ZeroPulseDelay>=0) Target.Start=ZeroPulseDelay+(StartScaling*SensorValue/1024); else Target.Start=StartScaling*SensorValue/1024);
				if (TimeScaling!=0)
					if (ZeroPulseTime>=0) Target.Time=ZeroPulseTime+(TimeScaling*SensorValue/1024); else Target.Time=TimeScaling*SensorValue/1024);
				if (PauseScaling!=0)
					if (ZeroPulsePause>=0) Target.Pause=ZeroPulsePause+(PauseScaling*SensorValue/1024); else Target.Pause=PauseScaling*SensorValue/1024);
			}
	}
